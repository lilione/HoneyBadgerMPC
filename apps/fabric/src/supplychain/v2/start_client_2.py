...
def hand_off_item_to_next_provider(input_provider, output_provider, item_id, prev_seq, prev_r, peer=0, org=1):
    ...
    start_time = time.perf_counter()
    Commitment().check_commit(input_provider, cur_r, commit_input_provider)
    end_time = time.perf_counter()
    ...
    for line in stdout.split(b'\n'):
        ...
        if "payload" in line:
            ...
            return seq, r, prove_time, check_time, finalize_time
...
if __name__ == '__main__':
    ...
    total_exe_time, total_check_time, total_finalize_time = 0, 0, 0

    for i in range(1, repetition + 1):
        seq, r, exe_time, check_time, finalize_time = hand_off_item_to_next_provider(i, i+1, item_id, seq, r)
        print(f"**** item_id {item_id} seq {seq} exe_time {exe_time} check_time {check_time} finalize_time {finalize_time}")
        total_exe_time += exe_time
        total_check_time += check_time
        total_finalize_time += finalize_time

    print(f"**** avg_exe_time {1. * total_exe_time / repetition}")
    print(f"**** avg_check_time {1. * total_check_time / repetition}")
