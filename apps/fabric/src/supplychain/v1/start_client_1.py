...
def register_item_global(idx_registrant, peer=0, org=1):
    env = os.environ.copy()
    cmd = ['docker', 'exec', 'cli', '/bin/bash', '-c', f"export CHANNEL_NAME=mychannel && bash scripts/run_cmd.sh 1_registerItemFinalizeGlobal {peer} {org} {idx_registrant}"]
    task = subprocess.Popen(cmd, env=env, stdout=subprocess.PIPE)
    task.wait()
...
def register_item_local(item_ID, seq, masked_registrant):
    env = os.environ.copy()

    tasks = []
    for peer in range(2):
        for org in range(1, 3):
            cmd = ['docker', 'exec', 'cli', '/bin/bash', '-c', f"export CHANNEL_NAME=mychannel && bash scripts/run_cmd.sh 1_registerItemFinalizeLocal {peer} {org} {item_ID} {seq} {masked_registrant}"]
            task = subprocess.Popen(cmd, env=env)
            tasks.append(task)
...
def register_item(registrant):
    inputmask_idx = get_inputmask_idx(1, 1)
    print("**** inputmask_idx", inputmask_idx)

    idx_registrant = inputmask_idx[0]
    item_ID, seq = register_item_global(idx_registrant)
    print(f"**** item_ID {item_ID} seq {seq}")
    ...
    for i in inputmask_idx:
        mask.append(asyncio.run(client.get_inputmask(int(i)))[0])
    masked_registrant = str(registrant + mask[0])[1:-1]
    register_item_local(item_ID, seq, masked_registrant)
...
def hand_off_item_start_local(idx_input_provider, masked_input_provider, idx_output_provider, masked_output_provider, item_ID, prev_seq, seq):
    env = os.environ.copy()
    tasks = []

    for peer in range(2):
        for org in range(1, 3):
            cmd = ['docker', 'exec', 'cli', '/bin/bash', '-c', f"export CHANNEL_NAME=mychannel && bash scripts/run_cmd.sh 1_handOffItemStartLocal {peer} {org} {idx_input_provider} {masked_input_provider} {idx_output_provider} {masked_output_provider} {item_ID} {prev_seq} {seq}"]
            task = subprocess.Popen(cmd, env=env)
            tasks.append(task)
...
def hand_off_item(input_provider, output_provider, item_ID, prev_seq):
    ...
    start_time = time.perf_counter()
    hand_off_item_start_local(idx_input_provider, masked_input_provider, idx_output_provider, masked_output_provider, item_ID, prev_seq, seq)
    wait_until_shipment_committed(item_ID, seq, "finalizeLocal")
    end_time = time.perf_counter()
...
if __name__ == '__main__':
    ...
    for i in range(1, repetition + 1):
        seq, time_get_inputmask_idx, time_get_seq, time_calc_inputmask, time_finalized = hand_off_item(i, i+1, item_id, seq)
