...
def hand_off_item_finalize_global(idx_input_provider, idx_output_provider, item_ID, prev_seq, seq, peer, org):
    wait_until_shipment_committed(item_ID, seq, "startLocal")

    env = os.environ.copy()
    cmd = ['docker', 'exec', 'cli', '/bin/bash', '-c', f"export CHANNEL_NAME=mychannel && bash scripts/run_cmd.sh 1_handOffItemFinalizeGlobal {peer} {org} {idx_input_provider} {idx_output_provider} {item_ID} {prev_seq} {seq}"]
    task = subprocess.Popen(cmd, env=env)
    task.wait()

def hand_off_item_finalize_local(masked_input_provider, masked_output_provider, item_ID, seq, peer, org):
    wait_until_shipment_committed(item_ID, seq, "finalizeGlobal")

    env = os.environ.copy()
    cmd = ['docker', 'exec', 'cli', '/bin/bash', '-c', f"export CHANNEL_NAME=mychannel && bash scripts/run_cmd.sh 1_handOffItemFinalizeLocal {peer} {org} {masked_input_provider} {masked_output_provider} {item_ID} {seq}"]
    task = subprocess.Popen(cmd, env=env)
    task.wait()
...
if __name__ == '__main__':
    ...
    prev_seq = sys.argv[6]
    seq = sys.argv[7]
    share_prev_output_provider = sys.argv[8]
    # idx_amt = sys.argv[5]
    ...
    if asyncio.run(check_condition(local_host, idx_input_provider, masked_input_provider, share_prev_output_provider)):
        if local_peer == '0' and local_org == '1':
            hand_off_item_finalize_global(idx_input_provider, idx_output_provider, item_ID, prev_seq, seq, local_peer, local_org)
        hand_off_item_finalize_local(masked_input_provider, masked_output_provider, item_ID, seq, local_peer, local_org)
