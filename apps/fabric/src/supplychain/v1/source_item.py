...
def source_item_finalize_global(item_ID, seq, input_provider, peer=0, org=1):
    wait_until_inquiry_committed(item_ID, seq, "startLocal")

    env = os.environ.copy()
    cmd = ['docker', 'exec', 'cli', '/bin/bash', '-c', f"export CHANNEL_NAME=mychannel && bash scripts/run_cmd.sh 1_sourceItemFinalizeGlobal {peer} {org} {item_ID} {seq} {input_provider}"]
...
if __name__ == '__main__':
    item_ID = sys.argv[1]
    seq = sys.argv[2]
    share_input_provider = sys.argv[3]
    ...
    source_item_finalize_global(item_ID, seq, input_provider)